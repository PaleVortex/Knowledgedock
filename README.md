...
A lightweight Flask backend is included for exposing the database via HTTP. You
can start it separately or have the GUI start it automatically. Use the
`--run-backend` or `--remote` CLI options as shown below:

```bash
# start backend alone
python backend.py

# run GUI and backend together
python main.py --run-backend

# talk to an existing backend from GUI
