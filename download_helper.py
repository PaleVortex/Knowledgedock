...
class DownloadHelper:
    ...
    def download_book_from_openlibrary(self, book_key: str, progress_callback=None) -> tuple[bool, str]:
        ...
        try:
            ...
            if url:
                ...
                return self.download_file(url, f"{filename}.pdf", progress_callback)
            else:
    ...
    def download_from_gutenberg(self, book_id: str, format_type: str = 'epub', progress_callback=None) -> tuple[bool, str]:
        ...
        # Project Gutenberg download URL format (best-effort)
        supported_formats = {
            'epub': f"https://www.gutenberg.org/ebooks/{book_id}.epub.images",
            'html': f"https://www.gutenberg.org/files/{book_id}/{book_id}-h/{book_id}-h.htm",
            'txt': f"https://www.gutenberg.org/files/{book_id}/{book_id}.txt"
        }
