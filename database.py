...
class BookmarkManager:
    """Local bookmark manager using SQLite"""
    ...
    def get_all_bookmarks(self):
        """Get all bookmarks as tuples for backend serialization"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('''
...
class RemoteBookmarkManager:
    """Manager that talks to a running backend HTTP server instead of SQLite.

    The interface mirrors :class:`BookmarkManager` so the GUI can switch between
    local and remote storage transparently.
    """
    ...
    def add_bookmark(self, title: str, url: str, source: str = "",
                     resource_type: str = "", cover_url: str = "",
                     description: str = "") -> bool:
        """Add a new bookmark via remote API"""
        try:
            r = self._session.post(f"{self.base_url}/api/bookmarks", json={
                'title': title,
                'url': url,
                'source': source,
            ...
            print(f"Remote add_bookmark failed: {e}")
            return False

    def remove_bookmark(self, url: str) -> bool:
        """Remove a bookmark by URL via remote API"""
        try:
            resp = self._session.get(f"{self.base_url}/api/bookmarks")
            for bm in resp.json():
                if bm.get('url') == url:
                    bid = bm.get('id')
                    r = self._session.delete(f"{self.base_url}/api/bookmarks/{bid}")
                    return r.status_code in [200, 204]
            return False
        except Exception as e:
            print(f"Remote remove_bookmark failed: {e}")
            return False

    def get_all_bookmarks(self) -> list:
        """Fetch all bookmarks from remote API (returns dicts, not tuples)"""
        try:
            resp = self._session.get(f"{self.base_url}/api/bookmarks")
            return resp.json()
        except Exception as e:
            print(f"Remote get_all_bookmarks failed: {e}")
            return []

    def get_bookmarks_by_source(self, source: str):
        """Get bookmarks from a specific source"""
        all_bms = self.get_all_bookmarks()
        return [bm for bm in all_bms if bm.get('source') == source]

    def is_bookmarked(self, url: str) -> bool:
        """Check if URL is bookmarked"""
        all_bms = self.get_all_bookmarks()
        return any(bm.get('url') == url for bm in all_bms)

    def search_bookmarks(self, query: str) -> list:
        """Search bookmarks by title, description, or source"""
        all_bms = self.get_all_bookmarks()
        query_lower = query.lower()
        return [
            bm for bm in all_bms
            if (query_lower in (bm.get('title', '') or '').lower() or
                query_lower in (bm.get('description', '') or '').lower() or
                query_lower in (bm.get('source', '') or '').lower())
        ]

    def get_bookmark_count(self) -> int:
        """Get total number of bookmarks"""
        return len(self.get_all_bookmarks())
