...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             QHBoxLayout, QPushButton, QLabel, QStackedWidget, QSplashScreen)
...
try:
    from backend import start_in_thread as start_backend
except ImportError:
    start_backend = None  # backend module may not be available
# Helper function to get resource paths (works with both script and executable)
def get_resource_path(relative_path) -> str:
    """Get the path to a resource file, works in both development and built executable."""
    base_path: sys.Any | str = getattr(sys, '_MEIPASS', os.path.dirname(os.path.abspath(__file__)))
...
class KnowledgedockApp(QMainWindow):
    def __init__(self, use_remote: bool = False, remote_url: str | None = None) -> None:
        """Main window for the application.

        :param use_remote: if True, the bookmark manager will communicate with
            a running HTTP backend instead of using the local SQLite database.
        :param remote_url: base URL of the backend; ignored if use_remote is False.
        """
        super().__init__()
        self.setWindowTitle("Knowledgedock")
        
        # Load icon with proper path resolution for both script and executable
        icon_path: str = get_resource_path(os.path.join("assets", "cover.png"))
        ...
        if use_remote:
            from database import RemoteBookmarkManager
            self.bookmark_manager = RemoteBookmarkManager(remote_url or 'http://127.0.0.1:5000')
        else:
        ...
        self.extension_manager: ExtensionManagerCore = ExtensionManagerCore(str(EXTENSIONS_DIR))
        ...
        # Sidebar
        self.sidebar: QWidget = self.create_sidebar()
        ...
        self.bookmarks_view: BookmarksView[BookmarkManager] = BookmarksView(self.bookmark_manager)
        self.projects_view: ProjectsView[ProjectManager] = ProjectsView(self.project_manager)
        self.extensions_view: ExtensionsSettingsView[ExtensionManager, DatabaseManager] = ExtensionsSettingsView(self.extension_db_manager, self.db_manager, self.extension_manager)
    ...
    def setup_extensions(self) -> None:
        ...
        try:
            ...
            # Register in database (version numbers/hints should ideally come from each extension)
            ...
            logger.exception("Error setting up extensions: %s", e)
    ...
    def create_sidebar(self) -> QWidget:
        ...
        nav_buttons: list[tuple[str, str, Callable[[], None]]] = [
            ("🏠", "Home", self.show_home),
    ...
    def read_from_project(self, url, title) -> None:
    ...
    def handle_reader_bookmark(self, title, url, extension_name) -> None:
    ...
    def handle_add_to_project(self, title, url, extension_name) -> None:
...
def main() -> NoReturn:
    """Entry point for the application. Parses command-line options, configures
    environment, initializes QApplication, optionally shows splash screen, and
    launches the main window.
    """

    parser = argparse.ArgumentParser(description="Knowledgedock launcher")
    parser.add_argument("--no-splash", action="store_true", help="don't show splash screen")
    parser.add_argument("--debug", action="store_true", help="enable debug logging")
    parser.add_argument("--run-backend", action="store_true", help="start the HTTP backend server")
    parser.add_argument("--remote", action="store_true", help="use remote bookmark manager (requires backend)")
    ...
    # optional backend server launch
    if args.run_backend:
        if start_backend is not None:
            logger.info("starting backend server on http://127.0.0.1:5000")
            start_backend(debug=args.debug)
        else:
            logger.warning("backend module unavailable; cannot start server")

    # application-wide icon and splash should use resource helper
    cover_path: str = get_resource_path(os.path.join("assets", "cover.png"))
    if os.path.exists(cover_path):
        app_icon = QIcon(cover_path)
        app.setWindowIcon(app_icon)
    else:
        logger.warning("icon not found at %s", cover_path)

    splash = None
    if not args.no_splash:
        splash_pix = QPixmap(cover_path)
        if not splash_pix.isNull():
            splash_pix: QPixmap = splash_pix.scaled(400, 400, Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    window = KnowledgedockApp(use_remote=args.remote)
    if splash:
